...
function fa2AuctionCopilotSnapshot(context=fa2LiveContext("")){
  ...
  const legacyPromoted=!!top.meta?.dynamic,legacyTarget=!!top.meta?.legacyStaticTarget;
  const status=legacyPromoted?"ALTERNATIVA LEGACY":legacyTarget?"TARGET LEGACY":"MIGLIOR PROFILO DISPONIBILE";
  const why=legacyPromoted
    ? `${p.name} è l'alternativa già calcolata dal motore legacy per ${top.meta.dynamic?.lost?.name||"un TARGET perso"}; nessuno slot del Piano Strategia decide contemporaneamente su questo giocatore.`
    : legacyTarget
      ? `${p.name} è un TARGET legacy non gestito da uno slot salvato. Il limite operativo resta il MAX live.`
      : `${p.name} è il primo profilo dell'ordinamento Asta Live per la fase corrente; non viene presentato come TARGET del Piano Strategia.`;
  return {
    mode:"market",status,badge:legacyPromoted?"ALT":legacyTarget?"TARGET":"PROFILO",player:p,slotLabel:"Fuori piano",
...
function fa2PlayerIntelligenceHTML(p){
  ...
  return `<section class="pi-card ${status.className}">
    <div class="pi-head"><div><span>PLAYER INTELLIGENCE · ${esc(status.label)}</span><b>${Math.round(Number(data.score)||0)}/100 <small>affidabilità ${Math.round(Number(data.reliability)||0)}%</small></b><small>${esc(source)} · aggiornato ${esc(engine.formatAge())}</small><small class="pi-identity">IDENTITÀ: ${esc(identity.methodLabel||engine.methodLabel?.(identity.method)||"VERIFICATA")} · ${Math.round(Number(identity.confidence)||0)}%</small></div><button class="ghost pi-refresh" onclick='refreshPlayerIntelligenceProfile(${idArg(p.id)})'>Aggiorna statistiche</button></div>
    ...
    <div class="pi-source-note">Resolver identità A4.1 · fonte storica: ${esc(source)}. Titolarità probabile e Listone restano alimentati dai feed Fantacalcio già presenti.</div>
  </section>`;
}
...
async function fa2RosterImportSnapshot(){
  try{
    const response=await fetch(`./listone-current.json?t=${Date.now()}`,{cache:"no-store"});
    if(!response.ok)throw new Error(`HTTP ${response.status}`);
    const snapshot=await response.json();
    if(!syncSnapshotValid(snapshot))throw new Error("snapshot Listone non valido");
    return snapshot;
  }catch(error){
    if(syncSnapshotValid(appliedListoneSync))return appliedListoneSync;
    throw new Error(`Listone ufficiale non disponibile: ${error?.message||"controllo non riuscito"}.`);
  }
}
...
function fa2ResolveRosterCsv(parsed,snapshot){
  ...
  const teams=parsed.teams.map(team=>{
    ...
    const rows=team.rows.map(row=>{
      ...
      if(!player){missing.push(row);return {...row,player:null,internalId:""}}
    ...
    if(clubViolation)throw new Error(`${team.name}: ${clubViolation[1]} giocatori ${clubViolation[0]}, oltre il limite di ${clubLimit}. Verifica il Regulation Studio.`);
    return {...team,rows,remaining:configuredBudget()-team.spent};
  });
  if(missing.length){
    const ids=missing.slice(0,8).map(row=>row.officialId).join(", ");
    throw new Error(`${missing.length} ID non presenti nel Listone ufficiale (${ids}). Aggiorna il Listone e riprova.`);
  }
  return {teams,totalPlayers:parsed.totalPlayers,matched:parsed.totalPlayers,missing:0,snapshot};
}
...
function fa2UpdateRosterImportSelection(){
  ...
  if(button)button.disabled=ownKey==="";
...
function fa2RosterImportPreviewHTML(pending){
  ...
  return `<div class="dialog-body roster-import-dialog-body">
    ...
    <form id="rosterImportForm">
      ...
      <div class="roster-import-regulation"><b>Regolamento attivo: ${budget} crediti · rosa ${rosterTotal}</b><span>Il CSV contiene i prezzi, ma non il budget iniziale. Residui e MAX vengono calcolati dal Regulation Studio.</span></div>
...
function fa2RegulationCard(){
  ...
  return `<div class="card fa2-reg-settings-card"><div class="fa2-reg-settings-head"><div><span>FANTAASTA2.0 · REGULATION STUDIO</span><h3>Regolamento A4.1</h3><p>Un'unica configurazione alimenta Strategy Engine, budget e Asta Live. La migrazione conserva automaticamente il profilo v2.</p></div><b>α4.1</b></div>
...
function fa2ModuleAdvisorFingerprint(primaryId,secondaryId,reg){
  ...
  return JSON.stringify([
    ...
    appliedListoneSync?.generatedAt||"",formationsLiveFeed?.generatedAt||"",
    window.FA2PlayerIntelligence?.generatedLabel?.()||""
  ]);
}
...
function fa2RenderStrategyResult(result){
  ...
  const critical=`<div class="fa2-critical"><b>SLOT DA PROTEGGERE · TOCCA PER APRIRE</b><div>${(p.critical||[]).map(x=>`<button type="button" data-fa2-slot-open="${esc(firstInstanceFor(fa2SlotKey(x.roles)))}">${x.roles.join("/")} · rischio ${x.scarcity}% · ${x.strongCount} forti</button>`).join("")}</div></div>`;
  const slotLab=`<div class="fa2-slot-lab"><div><b>PIANO MULTI-SLOT</b><span>Ogni posizione è indipendente: TARGET, ALT 1-3, VALUE, minimo, budget e priorità.</span></div><div>${slotInstances.map(x=>`<button type="button" data-fa2-slot-open="${esc(x.id)}">${savedSlots[x.id]?"✓ ":""}${esc(x.label)}</button>`).join("")}</div></div>`;
  ...
  return `<div class="fa2-result">${advisorHTML}<div class="fa2-result-head"><div><span>${result.mode==="auto"?"AUTO · COPPIA CONSIGLIATA":result.mode==="dual"?"STRATEGIA DOPPIO MODULO":"STRATEGIA MONO MODULO"}</span><b>${p.module.name}${result.secondary?` + ${result.secondary.module.name}`:""}</b><small>${scopeLabel}${result.secondary?` · sinergia ${result.synergy||0}%`:""}</small></div><b class="fa2-score">${headlineScore}<small>/100</small></b></div>${kpis}${fa2RenderExplanation(p)}${budget}${critical}${slotLab}${savedPlan}${secondary}${ranking}</div>`;
}
function renderStrategyView(){
  ...
  const piDiagnostics=window.FA2PlayerIntelligence?.diagnostics?.(allPlayers)||null;
  ...
  $("#fa2Primary").onchange=e=>{FA2Strategy.saveProfile({...FA2Strategy.loadProfile(),primary:e.target.value});fa2InvalidateModuleAdvisor()};
